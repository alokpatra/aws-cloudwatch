...
if (!$report_mem_util && !$report_mem_used && !$report_mem_avail && !$report_mem_buffer && !$report_mem_cached
...
if ($report_mem_util || $report_mem_used || $report_mem_free || $report_mem_avail || $report_mem_buffer || $report_mem_cached || $report_swap_util || $report_swap_used)
{
  ...
  my $mem_buffer = $meminfo{'Buffers'} * KILO;
  ...
  if ($report_mem_buffer) {
    add_metric('MemoryBuffer', $mem_units, $mem_buffer / $mem_unit_div);
  }
